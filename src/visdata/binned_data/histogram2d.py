...
class Histogram2d:
    ...
    def _setup_marginal_subplots(self):
        ...
        # Calculate correct width and height from config
        a = self._marginal_grid["main_width_ratio"]
        b = 1
        c = self._marginal_grid["cbar_width_ratio"]
        d = self._marginal_grid["main_height_ratio"]
        ...
        cbar_spacing = self._marginal_grid["cbar_spacing"]
        cbar_width = c / (a + b + c) * figsize[0] - cbar_spacing

        # Create grid, add colorbar as inset because of spacing
        ...
        colorbar_ax = ax_hist_y.inset_axes([1 + cbar_spacing, 0, cbar_width, 1])
